...
def extract_frames_from_video(video_path: Path, output_path: Path, frame_interval: int, progress_callback):
    ...
    try:
        ...
        frames_dir = output_path / f"{video_name}_frames_{timestamp}"
        ...
        while True:
            ...
            if frame_id % frame_interval == 0:
                # Save the frame
                filename = frames_dir / f'frame_{saved_id:05d}.jpg'
                cv2.imwrite(str(filename), frame)
                logging.info(f"Saved frame {saved_id} to {filename}")
                saved_id += 1
                
                # Update progress
                progress_callback(100 * frame_id / total_frames)
                
            frame_id += 1
            
        cap.release()
        logging.info(f"Extracted {saved_id} frames from {video_path}")
        return frames_dir
        
    except Exception as e:
        logging.error(f"Failed to extract frames from {video_path}: {e}")
        return None

class PanoramaProcessorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Panorama and Video Frame Processor")
        ...
        # Create tabs
        self.panorama_tab = ttk.Frame(self.tab_control)
        self.video_tab = ttk.Frame(self.tab_control)
        
        self.tab_control.add(self.panorama_tab, text="Panorama Processing")
        self.tab_control.add(self.video_tab, text="Video Frame Extraction")
        
        self.create_panorama_widgets()
        self.create_video_widgets()
        self.setup_logging()
        self.create_profile_widgets()

    def create_panorama_widgets(self):
        # Create main frame with two columns
        main_frame = ttk.Frame(self.panorama_tab, padding="10")
        ...
        self.panorama_tab.columnconfigure(0, weight=1)
        main_frame.rowconfigure(0, weight=1)
    ...
    def create_video_widgets(self):
        # Create main frame
        main_frame = ttk.Frame(self.video_tab, padding="10")
        ...
        ttk.Label(file_frame, text="Video File:").grid(row=0, column=0, sticky="w", pady=5)
        self.video_path_var = tk.StringVar()
        self.video_path_entry = ttk.Entry(file_frame, textvariable=self.video_path_var, width=60)
        self.video_path_entry.grid(row=0, column=1, padx=5, sticky="we")
        ...
        self.video_output_entry = ttk.Entry(file_frame, textvariable=self.video_output_var, width=60)
        self.video_output_entry.grid(row=1, column=1, padx=5, sticky="we")
        ttk.Button(file_frame, text="Browse", command=self.browse_video_output).grid(row=1, column=2, padx=5)
        
        # Frame Extraction Settings
        settings_frame = ttk.LabelFrame(main_frame, text="Frame Extraction Settings", padding="10")
        settings_frame.pack(fill="x", pady=5)
        
        ttk.Label(settings_frame, text="Extract every N frames:").grid(row=0, column=0, sticky="w", pady=5)
        self.frame_interval_var = tk.IntVar(value=15)
        self.frame_interval_spin = ttk.Spinbox(settings_frame, from_=1, to=300, textvariable=self.frame_interval_var, width=10)
        self.frame_interval_spin.grid(row=0, column=1, padx=5, sticky="w")
        
        # Start Button and Progress
        control_frame = ttk.Frame(main_frame, padding="10")
        control_frame.pack(fill="x", pady=5)
        
        self.extract_button = ttk.Button(control_frame, text="Extract Frames", command=self.start_frame_extraction)
        self.extract_button.pack(pady=10)
        
        self.video_progress_var = tk.DoubleVar()
        self.video_progress_bar = ttk.Progressbar(control_frame, variable=self.video_progress_var, maximum=100, length=400)
        self.video_progress_bar.pack(fill="x", pady=5)
        
        # Log Frame
        log_frame = ttk.LabelFrame(main_frame, text="Log", padding="10")
        log_frame.pack(fill="both", expand=True, pady=5)
        
        self.video_log_text = tk.Text(log_frame, height=10, state='disabled')
        self.video_log_text.pack(fill="both", expand=True)
        
        # Process extracted frames checkbox
        process_frame = ttk.Frame(main_frame, padding="10")
        process_frame.pack(fill="x", pady=5)
        
        self.process_frames_var = tk.BooleanVar(value=False)
        self.process_frames_check = ttk.Checkbutton(
            process_frame, 
            text="Process extracted frames with panorama settings after extraction", 
            variable=self.process_frames_var
        )
        self.process_frames_check.pack(anchor="w")
    ...
    def setup_logging(self):
        ...
        video_handler = TextHandler(self.video_log_text)
    ...
    def browse_video_output(self):
        directory = filedialog.askdirectory()
        if directory:
            self.video_output_var.set(directory)
    ...
    def start_frame_extraction(self):
        ...
        # Disable extract button to prevent multiple clicks
    ...
    def extract_frames(self, video_path, output_dir, frame_interval):
        try:
            ...
            if frames_dir and self.process_frames_var.get():
                # If requested, process extracted frames with panorama settings
                logging.info("Processing extracted frames with panorama settings...")
                
                # Get parameters from panorama tab
                try:
                    args = {
                        'FOV': int(self.fov_var.get()),
                        'output_width': int(self.width_var.get()),
                    ...
                    self.process_images(
                        ...
                        output_dir=str(output_path / f"{video_path.stem}_processed_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"),
                        args=args,
                        num_workers=int(self.worker_var.get())
                    )
                except ValueError as ve:
                    logging.error(f"Failed to process extracted frames: {ve}")
            ...
            def show_output_location():
                if frames_dir:
                    ...
                    messagebox.showerror(
                        ...
                        "Failed to extract frames. See log for details."
                    )
    ...
    def save_profile(self):
        ...
        self.profiles[name] = {
            ...
            # Add video frame extraction settings
    ...
    def load_profile(self):
        ...
        # Load video frame extraction settings
        self.frame_interval_var.set(profile.get('frame_interval', 15))
        self.process_frames_var.set(profile.get('process_frames', False))
...
def main():
    ...
    root.geometry("1500x1500")  # Set initial window size
    root.mainloop()
